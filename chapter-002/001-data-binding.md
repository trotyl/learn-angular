...
虽然这里的新语法非常浮夸，也完全没有任何美感，不过现在我们的应用确实能够正常使用了。不过，虽然插值语法可以自由配置，但是大多数时候完全没有必要，在本文的其它部分以及其它文章中如无特殊说明的情况下均使用默认的插值语法。
...
实际上 Angular 自始至终都无法得知事件绑定的事件来源，该内容会在之后的部分覆盖。
